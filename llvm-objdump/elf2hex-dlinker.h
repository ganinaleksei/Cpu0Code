...
static void PrintSoDataSections(const ObjectFile *o, uint64_t& lastDumpAddr, 
  ...
  for (const SectionRef &Section : o->sections()) {
    ...
    if (Name == ".dynsym") {
      ...
      fclose(fd_num_dyn_entry);
      raw_fd_ostream fd_dynsym("dlconfig/dynsym", Error, sys::fs::F_Text);
      int count = 0;
      ...
      raw_fd_ostream fd_dynstr("dlconfig/dynstr", Error, sys::fs::F_Text);
      raw_fd_ostream fd_dynstrAscii("dlconfig/dynstrAscii", Error, 
                                    sys::fs::F_Text);
